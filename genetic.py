...
def fitness(vector, gifts):
    #local score(1M samples): mean: 35513.9, stdv: 315.456, max: 36771.6, score > 36100: 2.98%
    bags = [[] for i in range(BAGS)]
    for i in range(len(gifts)):
        if vector[i] > 0:
            bags[int(vector[i])-1].append(gifts[i])
    score, high, std = score_submission(bags)
    return (std + score + 50000.0 * high) * check_submission(bags)
...
def best_fit(gifts, capacity=40, nr_bags=1000):
...
def first_fit_2(gifts, capacity=45, nr_bags=1000):
    _gifts = deepcopy(gifts)
    random.shuffle(_gifts)

    horses = ['horse_'+str(i) for i in range(0,1000)]
    trains = ['train_'+str(i) for i in range(0,1000)]
    blocks = ['blocks_'+str(i) for i in range(0,1000)]
    random.shuffle(horses)
    random.shuffle(trains)
    random.shuffle(blocks)
    viable_bags = []
    for i in range(nr_bags):
        new_bag = [horses[i], trains[i], blocks[i]]
        gifts_weight = sum([ff_weight(gift) for gift in new_bag])
        viable_bags.append([new_bag, gifts_weight])
        for gift in new_bag: _gifts.remove(gift)
...
files = ['submission0.csv', 'Santa_03.csv', 'Santa_BTB.csv', 'submission_gen7500.csv', 'Santa_combo_search.csv',
         ...
         'submission_gen200.csv', 'submission_gen700.csv', 'submission_gen1000.csv', 'submission_gen1200.csv',
         'submission_gen1400.csv', 'submission_top.csv', 'jeroen1.csv', 'jeroen2.csv', 'jeroen3.csv', 'jeroen4.csv',
         'jeroen5.csv', 'jeroen6.csv', 'jeroen7.csv', 'jeroen8.csv', 'jeroen9.csv', 'submission_gen900.csv',
         'submission_gen1100.csv', 'submission_gen1300.csv']
...
def genetic(verbose=0):
    ...
    for i in range(int(POPULATION_SIZE/100)):
        ...
        ff_bags, ff_gifts = first_fit_2(gifts, nr_bags=BAGS)
        ff_individual = to_numeric_vector(ff_bags, gifts)
        print('FF2:', fitness(ff_individual, gifts))
        population.append((ff_individual, fitness(ff_individual, gifts)))

        bf_bags, bf_gifts = best_fit(gifts, nr_bags=BAGS)
        bf_individual = to_numeric_vector(bf_bags, gifts)
        print('BF:', fitness(bf_individual, gifts))
        population.append((bf_individual, fitness(bf_individual, gifts)))
    ...
    while 1:
        ...
        if verbose >= 1 and not generation % 5: print('Generation:', generation, '---', population[0][1],
                                                      '(Population size:', str(len(population))+')')
        if not generation % 50: write_submission_file(population[0][0], gifts, 'submission_gen'+str(generation)+'.csv')
...
def score_submission(submitted_bags, n=100, verbose=0):
    #local score(1M samples): mean: 35513.9, stdv: 315.456, max: 36771.6, score > 36100: 2.98%
    arr = np.zeros((n))
    rej = np.zeros((n))
    for i in range(n):
    ...
    high = float(sum([x > 36350 for x in list(arr)]))/float(n)
    if high > 0: print('high solution found')
    if verbose:
        print("mean of {} simulations: {}  std: {} ".format(n, arr.mean(), np.std(arr)))
        print("Reject rate: {}".format(rej.mean()))
        print("Score > 36350: ", high)
    return arr.mean(), high, np.std(arr)
